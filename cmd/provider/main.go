...
import (
	...
	"github.com/crossplane/provider-vkcloud/apis"
	"github.com/crossplane/provider-vkcloud/apis/v1alpha1"
	vkcloud "github.com/crossplane/provider-vkcloud/internal/controller"
	"github.com/crossplane/provider-vkcloud/internal/features"
	"github.com/crossplane/provider-vkcloud/internal/version"
)

func main() {
	var (
		app            = kingpin.New(filepath.Base(os.Args[0]), "VkCloud support for Crossplane.").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-vkcloud"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(ratelimiter.LimitRESTConfig(cfg, *maxReconcileRate), ctrl.Options{
		...
		LeaderElectionID:           "crossplane-leader-election-provider-vkcloud",
		LeaderElectionResourceLock: resourcelock.LeasesResourceLock,
		LeaseDuration:              func() *time.Duration { d := 60 * time.Second; return &d }(),
		RenewDeadline:              func() *time.Duration { d := 50 * time.Second; return &d }(),
	})
	kingpin.FatalIfError(err, "Cannot create controller manager")
	kingpin.FatalIfError(apis.AddToScheme(mgr.GetScheme()), "Cannot add VkCloud APIs to scheme")
	...
	if *enableChangeLogs {
		...
		clo := controller.ChangeLogOptions{
			ChangeLogger: managed.NewGRPCChangeLogger(
				...
				managed.WithProviderVersion(fmt.Sprintf("provider-vkcloud:%s", version.Version))),
		}
		o.ChangeLogOptions = &clo
	}

	kingpin.FatalIfError(vkcloud.Setup(mgr, o), "Cannot setup VkCloud controllers")
